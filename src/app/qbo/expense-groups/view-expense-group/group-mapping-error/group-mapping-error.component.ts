...
import { MappingError } from 'src/app/core/models/mappingError.model';
...
export class GroupMappingErrorComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    that.isLoading = true;
    that.taskService.getTasksByExpenseGroupId(that.workspaceId, that.expenseGroupId).subscribe((res: Task[]) => {
