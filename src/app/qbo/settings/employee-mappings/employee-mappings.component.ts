...
export class EmployeeMappingsComponent implements OnInit {
  ...
  constructor(public dialog: MatDialog, private modalService: NgbModal, private route: ActivatedRoute, private mappingsService: MappingsService, private formBuilder: FormBuilder) {
  ...
  open(content) {
    ...
    const dialogRef = this.dialog.open(EmployeeMappingsDialogComponent, {
      width: '450px',
      data: {
      }
    });
  ...
  ngOnInit() {
    this.route.parent.params.subscribe(params => {
      ...
      if (this.generalSettings.corporate_credit_card_expenses_object) {
        this.columnsToDisplay.push('ccc');
      }
    });
  }
}
